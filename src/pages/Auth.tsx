...
const Auth: React.FC = () => {
    ...
    const maxSteps = 4;
    const progress = mode === 'unlock' ? 0 : (stepNum / maxSteps) * 100;
    ...
    const cardClass = "rounded-xl border border-neutral-800/70 bg-neutral-900/40 p-5 sm:p-6 backdrop-blur-sm";
    ...
    return (
        <div className="relative min-h-screen w-full overflow-x-hidden bg-neutral-950 text-neutral-100">
            ...
            <main className="relative z-10 mx-auto max-w-6xl px-4 sm:px-6 pt-16 sm:pt-24 pb-32 sm:pb-44 md:pt-32">
                ...
                <div className="relative lg:grid lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-8 pb-14">
                    <div>
                        <div className="mb-4 flex flex-col gap-2 text-xs">
                            <div className="flex items-center justify-between">
                                <span className="rounded-full border border-neutral-800 bg-neutral-900/60 px-2 py-1 text-neutral-400">
                                    {mode === 'unlock' ? 'Unlock' : `Step ${stepNum} of ${maxSteps}`}
                                </span>
                            ...
                            {mode !== 'unlock' && (
                                <div className="h-1 w-full rounded bg-neutral-800 overflow-hidden">
                                    <div
                                        className="h-full transition-all duration-300 bg-gradient-to-r from-orange-600 via-orange-500 to-orange-400"
                            ...
                            <div className={cardClass}>
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        className="w-full rounded-md bg-neutral-900/60 border border-neutral-800 px-3 py-2 pr-10 text-sm text-neutral-200 placeholder:text-neutral-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500/30"
                                    />
                                    <button
                                        type="button"
                                        onClick={() => setShowUnlockPass(v => !v)}
                                        className="absolute inset-y-0 right-0 px-2 text-neutral-500 hover:text-orange-300"
                                        aria-label={showUnlockPass ? 'Hide passcode' : 'Show passcode'}
                                    >
                                ...
                                <div className="mt-3">
                                    <button
                                        ...
                                        className="w-full md:w-auto rounded-md bg-orange-500 px-4 py-2 text-xs font-semibold text-neutral-900 shadow hover:bg-orange-400 disabled:opacity-60"
                                    >
                        ...
                        {mode === 'new' && (
                            <form onSubmit={handleSubmit(handleRegister)} className={cardClass}>
                                <div className="mb-3 flex items-center justify-between">
                                ...
                                <div className="mt-2">
                                    <button
                                        type="submit"
                                        disabled={genLoading}
                                        className="w-full md:w-auto rounded-md bg-orange-500 px-4 py-2 text-xs font-semibold text-neutral-900 shadow hover:bg-orange-400 disabled:opacity-60 disabled:hover:bg-orange-500"
                                    >
                                        {genLoading ? 'Please wait…' : 'Continue'}
                                    </button>
                                    {isPgpArmored && (
                        ...
                        {mode === 'show' && generatedMnemonic && (
                            <div className={cardClass}>
                                ...
                                <div className="mt-4 rounded-md border border-neutral-800 bg-neutral-900 p-3">
                                    ...
                                    <div className="grid gap-2 sm:grid-cols-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            <button
                                                ...
                                                className="absolute inset-y-0 right-0 px-2 text-neutral-500 hover:text-orange-300"
                                                aria-label={showPass2 ? 'Hide passcode confirmation' : 'Show passcode confirmation'}
                                            >
                                    ...
                                    <div className="mt-2 flex items-center gap-3">
                                        <div className="flex-1 h-1.5 rounded bg-neutral-800 overflow-hidden">
                                            <div
                                                className={`h-full transition-all duration-300 ${passStrengthColors[passStrength]}`}
                                                style={{ width: `${(passStrength / 4) * 100}%` }}
                                            />
                                        </div>
                                        <span className="text-[10px] text-neutral-400">{passcode ? passStrengthLabel : 'Strength'}</span>
                                    </div>
                                ...
                                <div className="mt-4 flex flex-wrap gap-2 hidden md:flex">
                                    <button
                                        type="button"
                                        onClick={async () => {
                                            try {
                                        ...
                                        className="inline-flex items-center gap-2 rounded-md border border-neutral-700 bg-neutral-800/60 px-3 py-1.5 text-xs font-semibold text-neutral-200 hover:border-orange-500/50 hover:text-orange-200 transition"
                                    >
                    ...
                    <aside className="mt-10 lg:mt-0 space-y-4">
                        <div className={cardClass}>
                            ...
                            <p className="mb-3 text-[11px] text-neutral-500 leading-relaxed">In‑context alias generation and autofill.</p>
            ...
            {/* Sticky mobile action bar for mnemonic save */}
            {mode === 'show' && generatedMnemonic && (
                <div className="md:hidden fixed inset-x-0 bottom-0 z-30 bg-neutral-950/95 backdrop-blur border-t border-neutral-800 px-4 pt-3 pb-[calc(env(safe-area-inset-bottom)+12px)]">
                    <div className="flex gap-2">
                        <button
                            type="button"
                            onClick={async () => {
                            ...
                            className="flex-1 inline-flex items-center justify-center gap-2 rounded-md border border-neutral-700 bg-neutral-800/60 px-3 py-2 text-xs font-semibold text-neutral-200 hover:border-orange-500/50 hover:text-orange-200 transition"
                        >
                        ...
                        <button
                            ...
                            disabled={!canEncrypt}
                            onClick={handleEncryptContinue}
                            className="flex-[2] rounded-md bg-orange-500 px-4 py-2 text-xs font-semibold text-neutral-900 shadow hover:bg-orange-400 disabled:opacity-60"
                        >
                            {savingEnc ? 'Saving…' : 'Encrypt & Continue'}
                        </button>
                    </div>
                </div>
            )}
